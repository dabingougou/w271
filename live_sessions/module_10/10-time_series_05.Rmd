...
- Spurious regression
- Cointegration
- Multivariate Time Series Models: Vector Autoregression (VAR)
...
Analyzing multiple time series can pose a couple challenges:
  
  (1) A single time series can be influenced by lags of itself or lags of other time series

  (2) If the time series are non-stationary, a problem known as spurious regression may arise

We can address (1) with two types of models:

   1- The DL(q) (Distributed Lag) Model: 
   
   $$Y_t = \alpha + \beta_0 X_t + \beta_1 X_{t-1}+... + \beta_q X_{t-q} + \epsilon_t$$
   
Here, the effect of a time series variable is modeled using lags of other time series.
...
- Recall that if two time series are cointegrated, they remain close to each other in the long term. This is because the spread (OLS residual) between them $z_t = y_{1t}-\beta y_{2t}$ is a stationary time series and mean-reverting.

- This mean-reverting property of the residual / spread can be exploited for trading, and it is commonly referred to as “pairs trading” or “statistical arbitrage.” The idea behind pairs trading is to: 
   
  - Assume that the spread $z_t = y_{1t}-\beta y_{2t}$ is stationary or mean-reverting with zero mean:

  - if spread is low $(z_t < -s_0)$, then stock 1 is undervalued and stock 2 overvalued (we expect stock 1 to increase and/or stock 2 to decrease to make the spread return to zero), so we can (1) buy the spread (i.e., buy stock 1 and short-sell stock 2) and unwind the positions when it reverts to zero after i time steps $z_{t+i} = 0$.
       
  - if spread is high $(z_t > s_0)$, then stock 1 is overvalued and stock 2 undervalued (we expect stock 2 to increase and/or stock 1 to decrease to make the spread return to zero), so we can short-sell the spread (i.e., short-sell stock 1 and buy stock 2) and unwind the positions when it reverts to zero after i time steps $(z_{t+i} = 0)$.
...
b) Plot NFLX and AMZN closing prices, the ACF/PACF, and examine if they are stationary.

C) Carry out unit root tests for NFLX and AMZN closing prices and decide if any transformations, including taking logs, are necessary. Are the series both integrated to the same order?

d) Carry out the Engle-Granger test for cointegration (you can use functions like `ur.df` or `po.test` on the residuals).
...
- These intuitive ideas can be investigated through regression models incorporating the notion of Granger or regressive causality. The basic idea is that a variable X Granger causes Y if past values of X can help explain Y (and not vice versa).
...
- The variables in a VAR model should be stationary. If the original variables have unit roots, then we assume that differences have been taken to make the VAR variables stationary.
...
## Analytical Exercise (Technical - Feel Free to Skip Over)
...
e) Determine the lag length of the VAR using the information criteria. Estimate the VAR and comment on the fit. Use `VARselect` and `VAR`.

f) Test for Granger-causality. Does google trend Granger-cause bitcoin prices? Does bitcoin prices Granger-cause google trend?  Use `vars::causality`.

g) Do diagnostic checking of the VAR model using `serial.test`. 

h) We finally conduct a 3-step ahead forecast using `forecast`:

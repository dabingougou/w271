...
**Use `adf.test()` to run the ADF test on the simulated time series.**
...
**Use `pp.test()` to run the Philips Perron test on the simulated time series.**
...
## Case Study: Choosing the Number of Lags: Finding p, d, and q
...
# 
# fabletools::accuracy(model.forecasts, dat)
```

...
This part creates new, more usable variables from the original one-hot
encoded and confusingly named columns.

speed_limit: This new variable aggregates the maximum speed limit from
...
year_of_observation: This variable converts the year indicators (d80
through d04) into a single numerical year column (1980 through 2004),
making time-series analysis straightforward.

legal_bac_limit: This variable consolidates the Blood Alcohol Content
limit indicators (bac08, bac10) into a single numerical column (0.08 or
0.10).

Variable Renaming: All original variable names are renamed to be more
descriptive and legible (e.g., totfatrte becomes total_fatalities_rate,
vehicmiles becomes vmt_billion). This significantly improves the clarity
...
df <- data %>%
  ...
  mutate(
    legal_bac_limit = case_when(
      bac08 == 1 ~ 0.08,
      bac10 == 1 ~ 0.10,
      .default = NA_real_ # Should capture any unclassified BAC limits
    )
  ) %>%
...
**Structure** The data has `r dim(df)[1]` rows, and including
...
df %>%
  ...
  labs(
    title = 'Distribution of Total Fatalities Rate with Quantiles',
...
df %>%
  ...
  # Optional: Add the overall population-weighted average trend line (in black)
  # You would need the 'weighted_avg_rate_by_year' object calculated previously.
  # geom_line(data = weighted_avg_rate_by_year, aes(y = weighted_avg_rate), 

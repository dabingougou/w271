...
# Fit AR(p) models (assuming d=0)
fit_ar_models <- 
  unemployment %>%
#  pull(value) %>%
  model(
    auto_ar = ARIMA(value ~ 1 + pdq(D=0, Q=0)), # auto_arima with no differencing or MA
#    ar_1 = ARIMA(unemployment_rate ~ pdq(1, 0, 0)),
#    ar_2 = ARIMA(unemployment_rate ~ pdq(2, 0, 0)),
#    ar_3 = ARIMA(unemployment_rate ~ pdq(3, 0, 0))
  )
